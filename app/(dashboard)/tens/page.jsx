'use client';
/**
 * app/(dashboard)/tens/page.jsx
 *
 * Dashboard TENS.
 * Tabs:
 *   1. Gestión Inventario (TablaInsumos + modales ingreso/merma)
 *   2. Solicitudes pendientes (autorizar/despachar/rechazar)
 *   3. Consultar Stock (vista de stock, solo lectura)
 *   4. Mis Recepciones
 */
...
export default function TensDashboard() {
    ...
    const cambiarTab = (id) => {
        router.push(`/tens?tab=${id}`, { scroll: false });
    };
    ...
    const [stats, setStats] = useState({
        ...
        solPendientes: 0, solAutorizadas: 0, misDespachos: 0,
    ...
    const cargarStats = useCallback(async () => {
        ...
        const [
            ...
            supabase.from('solicitudes').select('*', { count: 'exact', head: true }).eq('gestionado_por', perfilId).eq('estado', 'despachada'),
            supabase.from('solicitudes_compra').select('estado').eq('solicitante_id', perfilId),
        ]);
        ...
        setStats({ totalInsumos, bajoStock, solPendientes, solAutorizadas, misDespachos, compraPendiente, compraAprobada, compraRechazada });
    }, [perfilId]);
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-slate-800">Panel TENS — {perfil?.nombre}</h1>
            ...
            {tabActiva !== 'inicio' && (
                <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
                    ...
                    <h2 className="text-base font-semibold text-slaate-700">{SECCION_LABEL[tabActiva]?.label}</h2>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                        ...
                        <button onClick={() => cambiarTab('solicitudes')}
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="flex justify-between text-sm">
                                    <span className="text-slate-500">Mis despachos realizados</span>
                                    <span className="font-semibold text-green-600">{stats.misDespachos}</span>
                                </div>
                            </div>
                        </button>
