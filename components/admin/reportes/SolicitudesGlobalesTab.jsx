'use client';
/**
 * components/admin/reportes/SolicitudesGlobalesTab.jsx
 *
 * Sub-tab "Solicitudes Globales" del panel de reportes administrador.
 * Listado completo de todas las solicitudes con filtros por estado,
 * rango de fechas y búsqueda libre. Se actualiza en tiempo real.
 */
...
export default function SolicitudesGlobalesTab() {
    ...
    const filtradas = solicitudes.filter((s) => {
        ...
        if (filtroBusqueda) {
            ...
            const inGestor = s.gestionado_por?.nombre?.toLowerCase().includes(q);
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                            ...
                            {loading ? (
                                ...
                                return (
                                    <tr key={s.id} className="hover:bg-slate-50 transition">
                                        ...
                                        <td className="px-4 py-3 text-slate-400 text-xs whitespace-nowrap">
                                            {ultimaFecha ? fmt(ultimaFecha) : <span className="text-slate-300">—</span>}
                                        </td>
                                    </tr>
                                );
                            })}
                ...
                {!loading && filtradas.length > 0 && (
                    <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 text-xs text-slate-400 flex items-center justify-between">
                        <span>Mostrando {filtradas.length} de {solicitudes.length} solicitudes</span>
                        <span>Tiempo real activo — actualiza automáticamente</span>
                    </div>
                )}
            </div>
        </div>
